...
export default function LiveKitVoiceRoom({ onTurnLogged }) {
  ...
  const [roomName, setRoomName] = useState('trustmoss-demo-room');
  const [participantId, setParticipantId] = useState(`voice-user-${Math.floor(Math.random() * 1000)}`);
  ...
  // Animated waveform effect when in room
  useEffect(() => {
    if (!inRoom) return;
  ...
  // Connect to LiveKit room and get WebRTC token
  ...
  // Submit spoken transcript to Trust Gateway
  ...
  return (
    <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-5 shadow-2xl flex flex-col gap-5">
      ...
      <div className="flex items-center justify-between border-b border-slate-800 pb-4">
        ...
        {/* Room Connect / Disconnect Button */}
        <div>
          {!inRoom ? (
            <button
      ...
      {/* Active Turn Telemetry & Circuit Breaker HUD */}
      {activeTurn && (
        <div className="flex flex-col gap-3 bg-[#080d16] border border-slate-800 rounded-xl p-4">
          ...
          {/* Transcript & Synthesized Response */}
